...
### Step 2.5: 글자수 게이트 (오케스트레이터 직접 수행)

Phase 2 완료 후, Phase 3 진입 전에 오케스트레이터가 직접 글자수를 측정한다.

**측정 기준**: `wc -m`으로 파일 전체 글자수를 측정한다 (마크다운 헤더·메타데이터 포함).
이 기준은 Step 4 교차검증의 `wc -m`과 동일하다. 일관된 측정 기준을 위해
전체 파일 글자수를 사용하며, 본문만 분리하지 않는다.

1. **측정**: Bash `wc -m {EPISODE_DIR}/ep{NNN}.md` → 글자수 확인
2. **판정**: 목표 범위(기본 6000-8000) 대비 확인
   - 범위 내: Phase 3으로 진행
   - **하한 미달** (예: 5,200자 < 6,000자):
     ...
     - 프롬프트에 다음을 포함:
       ...
       - 목표 하한: {MIN}자
       - 부족분: {MIN - N}자
       - 지시: 기존 장면을 확장하거나 부족한 묘사/대화를 보강하여 {MIN}자 이상으로 만들어라.
               새로운 플롯 비트를 추가하지 말고, 기존 장면의 밀도를 높여라.
       ```
     - 보정 후 다시 글자수를 측정한다. 2회까지 시도, 이후 Phase 3으로 진행
   - **상한 초과** (예: 9,500자 > 8,000자):
     - 경고만 출력하고 Phase 3으로 진행 (상한 초과는 REWRITE 사유가 아님)
...
3. Bash: `wc -m {EPISODE_DIR}/ep{NNN}.md` → 목표 글자수 범위(기본 6000-8000) 확인
...
```markdown
## create 설정
target_chars: 6000-8000          # 에피소드 당 목표 글자 수 (기본: 6000-8000)
